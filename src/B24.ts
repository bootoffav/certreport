...
class B24 {
  ...
  static formTaskFields = (state: any) => {
    ...
    const taskFields: any = {
      ...
      DESCRIPTION:
        ...
        `${
          state.startedOn &&
          `[B]tests to be started on:[/B] ${state.startedOn}\n`
        }` +
        `${
          stAd.testFinishedOn &&
  ...
  static fileUpload(taskId: string | undefined, name: string, content: string) {
    if (taskId) {
      return fetch(
  ...
  static async get_products() {
    ...
    for (let [sectionId, brand] of productSections) {
      do {
        productsInSection = productsInSection.concat(
          await fetch(
            `${main_url}/${creator_id}/${webhook_key}/crm.product.list?` +
              qs.stringify({
                order: {
  ...
  static get_product(id = null) {
    ...
    return fetch(
      `${main_url}/${creator_id}/${webhook_key}/crm.product.get?id=${id}`
    )
      .then((res) => res.json())
