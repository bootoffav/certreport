  ...
  static formTaskFields = (state : IState) => {
    ...
    const taskFields: any = {
      ...
      TITLE: `${state.serialNumber}_${state.testingCompany} - ${state.standards} (${state.pretreatment1}) - ${state.article}, ${state.colour} ` +
          `(send ${state.sentOn} - plan ${state.testFinishedOnPlanDate}) = ${(+state.price + +state.price2).toLocaleString('ru-RU', { style: 'currency', currency: 'EUR' })} | ${stAd.getStageForTitle()}${stAd.getNADForTitle()}`,
