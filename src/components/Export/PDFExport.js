...
export default class PDFExport {
  constructor(data) {
    ...
    let rows = [
      [1, data.article, data.product, data.code, data.colour,
        data.length, data.width, data.partNumber, data.rollNumber],
      ];
  
      let pdf = new jsPDF('l');
      pdf.setFont('Calibri', 'normal');
      pdf.text(`NAME OF APPLICANT: ${data.applicantName}`, 60, 30);
      let table_options = {
        margin: {
          left: 15,
          width: 15
    ...
    pdf.setFontSize(12);
    pdf.text(`Standards: ${data.standards}`, 15, 80);
    pdf.text(`Testing company: ${data.testingCompany}`, 15, 85);
    pdf.text(`Materials needed: ${data.materialNeeded}`, 15, 90);
    pdf.text(`Testing Time: ${data.testingTime}`, 15, 95);
    pdf.text(`Sent on: ${data.formatDate(data.sentOn)}`, 15, 100);
    pdf.text(`Received on: ${data.formatDate(data.receivedOn)}`, 15, 105);
    pdf.text(`Started on: ${data.formatDate(data.startedOn)}`, 15, 110);
    pdf.text(`Finished on: ${data.formatDate(data.finishedOn)}`, 15, 115);
    pdf.text(`Results received on: ${data.formatDate(data.resultsReceived)}`, 15, 120);
