...
class PDF {
  ...
  constructor(
    ...
    this.data = new StateAdapter(data);
    this.pdf = new jsPDF('l');
    ...
    let rows = [
      [1, this.data.article, this.data.product, this.data.code, this.data.colour,
        this.data.length, this.data.width, this.data.partNumber, this.data.rollNumber],
      ];
    ...
    this.pdf.text(`NAME OF APPLICANT: ${this.data.applicantName}`, 60, 30);
    this.pdf.setFontSize(12);
    this.pdf.text(`Standards: ${this.data.standards}`, 15, 80);
    this.pdf.text(`Testing company: ${this.data.testingCompany}`, 15, 85);
    this.pdf.text(`Materials needed: ${this.data.materialNeeded}`, 15, 90);
    this.pdf.text(`Testing Time: ${this.data.testingTime}`, 15, 95);
    this.pdf.text(`Sent on: ${this.data.formatDate(this.data.sentOn)}`, 15, 100);
    this.pdf.text(`Received on: ${this.data.formatDate(this.data.receivedOn)}`, 15, 105);
    this.pdf.text(`Started on: ${this.data.formatDate(this.data.startedOn)}`, 15, 110);
    this.pdf.text(`Finished on: ${this.data.formatDate(this.data.finishedOn)}`, 15, 115);
    this.pdf.text(`Results received on: ${this.data.formatDate(this.data.resultsReceived)}`, 15, 120);
