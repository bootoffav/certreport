...
class Task {
    ...
    parse(description: string, ufCrmTask: string[]) {
        ...
        parsedState.brand = ufCrmTask.filter((v : any) => ['C_10033', 'C_10035', 'C_10037', 'C_10041'].includes(v)).join();
        // @ts-ignore
        parsedState.brand = brand.find(el => el.value === parsedState.brand).label;
